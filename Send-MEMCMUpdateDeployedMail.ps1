<#
	...
	.EXAMPLE
		Send-MEMCMUpdateDeployedMail.ps1
...
# Numbers of days back for selection of updates
$LimitDays = '-25'

# The name for the updategroup in MEMCM
...
write-host -ForegroundColor Green 'Configmgr module already imported and installed!'
...
Get-CMModule
Write-Log -LogString "Send-UpdateDeployedMail - CMmodule imported"
$sitecode = get-cmsitecode
Write-Log -LogString "Send-UpdateDeployedMail - $sitecode extracted"
$SetSiteCode = $sitecode + ":"
Set-Location $SetSiteCode
Write-Log -LogString "Send-UpdateDeployedMail - set location to $SetSiteCode"
...
if ($todayCompare -eq $ReportdayCompare)
{
	...
	Write-Log -LogString "Send-UpdateDeployedMail - Processing Software Update Group $UpdateGroupName"
		...
		$result += $object
            ...
            New-HTML -TitleText "Patchfönster- Kriminalvården" -FilePath $HTMLFileSavePath -ShowHTML -Online {
                ...
                New-HTMLSection -Invisible -Title "Maintenance Windows $filedate"{
                    ...
                    New-HTMLTable -DataTable $Result -PagingLength 25 -Style compact
...
"ALT="Kriminalvården logo"> 
<p><h1>Månadens uppdateringar</h1><br>
<p>Följande uppdateringar är släppta mot sevrar denna månaden.</p>
<p>Totalt så är det <B>$Numbersofupdates</b></p>
"@
...
"@
	...
	Write-Log -LogString "Send-UpdateDeployedMail - Mail on it´s way to $RecipientList"
	set-location $PSScriptRoot
	Write-Log -LogString "Send-UpdateDeployedMail - Script end!"
}
